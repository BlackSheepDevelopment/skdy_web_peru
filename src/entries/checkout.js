    ...
    switch (current_step) {
        case 'shipping-step':
            ...
            if (billing_district.val() === '') {
                window.alert('Debes seleccionar el distrito.');
                break;
            } else {
                shipping.hide();
                billing.fadeIn();
...
window.province_selector_init = function () {
    ...
    selector.after(function () {
        return '<p class="form-row shipping-step validate-required form-row-wide" id="billing_province_field">' +
    ...
    billing_district.on('change', function () {
        ...
        city.val(billing_province.val() + ' - ' + billing_district.val()).trigger('change');
    });
