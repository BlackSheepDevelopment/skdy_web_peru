...
$('.box-info, .more-tech-specs').find('.title').click(function (event) {
    event.preventDefault();
...
function renderSlider() {
    ...
    $('#color').find('option').map(function (i, e) {
        if ($(e).is(':selected')) {
            ...
            $('#product-header, #product-header .container').css('background-color', gallery.background);
            gallery.images.map(function (e) {
                console.log(e);
                $('.background-slider').slick('slickAdd', '' +
                    '<article>' +
                    '  <div>' +
                    '    <picture>' +
                    '      <source srcset="' + e['desktop'] + '" media="(min-width: 551px)" />' +
                    '      <img src="' + e['mobile'] + '">' +
                    '    </picture>' +
                    '  </div>' +
                    '</article>'
